#include "aes.h"
#include <math.h>


static const uint8_t sbox[256] = {
    
};

static void SubBytes(uint8_t state[4][4]) {

    for(int r=0;r<4;r++){
        for(int c=0;c<4;c++){
            state[r][c]=sbox[state[r][c]];
        }
    }

}

static void ShiftRows(uint8_t state[4][4]) { 
    
    uint8_t temp[4];

    for(int r=1;r<4;r++){

        for(int c=0;c<4;c++){
            temp[c]=state[r][c];
        }

        for(int c=0;c<4;c++){
            state[r][c]=temp[(r+c)%4];
        }
    }
   
}

uint8_t XTime(uint8_t x) {
    return (x >> 7) ? ((x << 1) ^ 0x1B) : (x << 1);
}

static void MixColumns(uint8_t state[4][4]) { 
    for(int c = 0; c < 4; c++) {
        
        uint8_t s0 = state[0][c];
        uint8_t s1 = state[1][c];
        uint8_t s2 = state[2][c];
        uint8_t s3 = state[3][c];

        state[0][c] = XTime(s0) ^ (XTime(s1) ^ s1) ^ s2 ^ s3;
        state[1][c] = s0 ^ XTime(s1) ^ (XTime(s2) ^ s2) ^ s3;
        state[2][c] = s0 ^ s1 ^ XTime(s2) ^ (XTime(s3) ^ s3);
        state[3][c] = (XTime(s0) ^ s0) ^ s1 ^ s2 ^ XTime(s3);
    }
}

static void AddRoundKey(uint8_t state[4][4], const uint8_t *round_key) { 

    for(int c = 0; c < 4; c++) {
        for(int r = 0; r < 4; r++){

            state[r][c]^=round_key[4*c+r];
        }

    }

 }

// Tableau des constantes de tour (Rcon)
static const uint8_t Rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

static void KeyExpansion(const uint8_t *key, uint8_t *round_keys, int key_size) { 
    int Nk = key_size / 4; 
    int Nr = Nk + 6;       
    int i = 0;

    while (i < Nk) {
        round_keys[4 * i]     = key[4 * i];
        round_keys[4 * i + 1] = key[4 * i + 1];
        round_keys[4 * i + 2] = key[4 * i + 2];
        round_keys[4 * i + 3] = key[4 * i + 3];
        i++;
    }

    uint8_t temp[4];

    while (i < 4 * (Nr + 1)) {
        
        temp[0] = round_keys[4 * (i - 1)];
        temp[1] = round_keys[4 * (i - 1) + 1];
        temp[2] = round_keys[4 * (i - 1) + 2];
        temp[3] = round_keys[4 * (i - 1) + 3];

        
        if (i % Nk == 0) {
            
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;

            
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];

            
            temp[0] ^= Rcon[i / Nk];
        } 
       
        else if (Nk > 6 && i % Nk == 4) {
            
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];
        }

        
        round_keys[4 * i]     = round_keys[4 * (i - Nk)]     ^ temp[0];
        round_keys[4 * i + 1] = round_keys[4 * (i - Nk) + 1] ^ temp[1];
        round_keys[4 * i + 2] = round_keys[4 * (i - Nk) + 2] ^ temp[2];
        round_keys[4 * i + 3] = round_keys[4 * (i - Nk) + 3] ^ temp[3];

        i++;
    }
}

static const uint8_t rsbox[256] = {
    
};

// Multiplication générique dans GF(2^8) utilisant XTime
static uint8_t Multiply(uint8_t x, uint8_t y) {
    uint8_t result = 0;
    uint8_t temp = x;
    while (y) {
        if (y & 1) {
            result ^= temp;
        }
        temp = XTime(temp);
        y >>= 1;
    }
    return result;
}

static void InvSubBytes(uint8_t state[4][4]) { 
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] = rsbox[state[r][c]];
        }
    }
}

static void InvShiftRows(uint8_t state[4][4]) {
    uint8_t temp[4];
    
    
    for (int r = 1; r < 4; r++) {
       
        for (int c = 0; c < 4; c++) {
            temp[c] = state[r][c];
        }
        // Décalage vers la DROITE : on ajoute 4 avant le modulo pour éviter un indice négatif
        for (int c = 0; c < 4; c++) {
            state[r][c] = temp[(c - r + 4) % 4];
        }
    }
}

static void InvMixColumns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++) {
        uint8_t s0 = state[0][c];
        uint8_t s1 = state[1][c];
        uint8_t s2 = state[2][c];
        uint8_t s3 = state[3][c];

        // Multiplication par la matrice inverse : {0e}, {0b}, {0d}, {09}
        state[0][c] = Multiply(s0, 0x0e) ^ Multiply(s1, 0x0b) ^ Multiply(s2, 0x0d) ^ Multiply(s3, 0x09);
        state[1][c] = Multiply(s0, 0x09) ^ Multiply(s1, 0x0e) ^ Multiply(s2, 0x0b) ^ Multiply(s3, 0x0d);
        state[2][c] = Multiply(s0, 0x0d) ^ Multiply(s1, 0x09) ^ Multiply(s2, 0x0e) ^ Multiply(s3, 0x0b);
        state[3][c] = Multiply(s0, 0x0b) ^ Multiply(s1, 0x0d) ^ Multiply(s2, 0x09) ^ Multiply(s3, 0x0e);
    }
}


// ============================================================================
// Implémentation de l'API publique
// ============================================================================

void AES_init_ctx(AES_CTX *ctx, const uint8_t *key, AES_KeySize key_size) {
    int key_bytes; 
    
    if (key_size == AES_128) {
        ctx->rounds = 10;
        key_bytes = 16;
    } else if (key_size == AES_192) {
        ctx->rounds = 12;
        key_bytes = 24;
    } else { // AES_256
        ctx->rounds = 14;
        key_bytes = 32;
    }
    
    KeyExpansion(key, ctx->round_keys, key_bytes);
}

void AES_encrypt_block(const AES_CTX *ctx, const uint8_t *plaintext, uint8_t *ciphertext) {
    uint8_t state[4][4];

    // 1. Copier le plaintext dans la matrice d'état (colonne par colonne)
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            state[r][c] = plaintext[c * 4 + r];
        }
    }

    // 2. AddRoundKey initial (Clé n°0)
    AddRoundKey(state, &ctx->round_keys[0]);

    // 3. Boucle principale de 1 à (Nr-1) tours
    for (int round = 1; round < ctx->rounds; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, &ctx->round_keys[round * 16]); // On décale de 16 octets à chaque tour
    }

    // 4. Tour final (pas de MixColumns)
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, &ctx->round_keys[ctx->rounds * 16]);

    // 5. Copier l'état final dans ciphertext (colonne par colonne)
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            ciphertext[c * 4 + r] = state[r][c];
        }
    }
}

void AES_decrypt_block(const AES_CTX *ctx, const uint8_t *ciphertext, uint8_t *plaintext) {
    uint8_t state[4][4];

    // 1. Copier le ciphertext dans la matrice d'état (colonne par colonne)
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            state[r][c] = ciphertext[c * 4 + r];
        }
    }

    // 2. AddRoundKey initial (avec la TOUTE DERNIÈRE sous-clé)
    AddRoundKey(state, &ctx->round_keys[ctx->rounds * 16]);

    // 3. Boucle inverse : de (Nr-1) jusqu'à 1
    for (int round = ctx->rounds - 1; round > 0; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, &ctx->round_keys[round * 16]);
        InvMixColumns(state);
    }

    // 4. Tour final inverse (pas de InvMixColumns, on utilise la PREMIÈRE sous-clé)
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, &ctx->round_keys[0]);

    // 5. Copier l'état final dans plaintext (colonne par colonne)
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            plaintext[c * 4 + r] = state[r][c];
        }
    }
}